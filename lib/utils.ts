...
import { CURRENCY_SYMBOL } from "@/constants/app-default";
...
export function formatDistanceRange(min: number, max: number | null): string {
  ...
  return `${formatDistance(min)} – ${formatDistance(max)}`;
}

export function formatPrice(
  value: number | undefined | null,
  currencySign: string = CURRENCY_SYMBOL
): string {
...
export function formatPercent(value: number | undefined | null): string {
  const numericValue = Number(value ?? 0);
  const safeValue = Number.isFinite(numericValue) ? numericValue : 0;
  return `${safeValue % 1 === 0 ? safeValue.toFixed(0) : safeValue.toFixed(2)}%`;
}

export function formatMinutes(value: number | undefined | null): string {
  const numericValue = Number(value ?? 0);
  const safeValue = Number.isFinite(numericValue) ? numericValue : 0;
  return `${safeValue} min`;
}
